...
async def get_info():
    ...
    class Info:
        def __init__(self, token):
            self.token = token
        async def get_track_by_id(self, track_id):
            try:
    ...
    def change_setting(tglle: bool): # Выношу в отдельную функцию, чтобы иметь возможность запустить в отдельном потоке,
        if tglle:# ДВА способа добавления в автозапуск. Первый через добавление программы в папку автостарта. Второй через изменение реестра. Оба не требуют админских прав.
            try: # Автозапуск через добавление в папку автозапуска
                ...
                shortcut_path = os.path.join(os.getenv('APPDATA'), 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup', 'YandexMusicRPC.lnk')  # Определяем путь для ярлыка в автозагрузке
                ...
                winreg.SetValueEx(key, 'YandexMusicRPC', 0, winreg.REG_SZ, exe_path)  # Устанавливаем новый параметр в реестре с именем 'YandexMusicRPC' и значением пути к исполняемому файлу
                winreg.CloseKey(key)  # Закрываем ключ реестра
        else: # Удаляем оба метода
            # Удаляем ярлык из автозагрузки
            shortcut_path = os.path.join(os.getenv('APPDATA'), 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup', 'YandexMusicRPC.lnk')
...
def is_in_autostart(): # Функция, которая при запуске программы проверяет, есть ли программа в автозапуске. Используется при подгрузке стартовых параметров
    ...
    def is_in_startup():
        shortcut_path = os.path.join(os.getenv('APPDATA'), 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup', 'YandexMusicRPC.lnk')  # Определяем путь к ярлыку
...
def get_saves_settings(fromStart = False):
    ...
    if fromStart:
        log(f"Loaded settings: {Style.RESET_ALL}button_config = {button_config.name}, language_config = {language_config.name}", LogType.Update_Status)
...
def create_tray_icon():
    ...
    icon = pystray.Icon("YandexMusicRPC", tray_image, "YandexMusicRPC", menu=pystray.Menu(
        pystray.MenuItem("Hide/Show Console", toggle_console, default=True),
        pystray.MenuItem('Start with Windows', toggle_auto_start_windows, checked=lambda item: auto_start_windows),
        pystray.MenuItem("Yandex settings", settingsMenu),
        pystray.MenuItem("RPC settings", rpcSettingsMenu),
...
def Is_already_running():
    hwnd = win32gui.FindWindow(None, "YandexMusicRPC - Console")
    if hwnd:
        return True
    return False
